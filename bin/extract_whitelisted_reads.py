...
def write_strain_fastas(reads_by_taxid, sample_id, output_dir="."):
    ...
    for taxid, reads in reads_by_taxid.items():
        ...
        output_file = os.path.join(output_dir, f"{sample_id}-{taxid}.taxon_specific.fasta")
...
def main():
    ...
    parser.add_argument('--taxid', type=int, required=True, 
                       help="Parent taxid to expand")
    parser.add_argument('--nodes', required=True, 
                       help="Path to nodes.dmp")
    parser.add_argument('--fasta', required=True, 
                       help="Path to extracted FASTA")
    parser.add_argument('--output', required=True, 
                       help="Output FASTA with all matching reads")
    parser.add_argument('--taxid-list', required=True, 
                       help="Path to write list of descendant strain taxids")
    parser.add_argument('--sample-id', required=True,
                       help="Sample ID for naming output files")
    parser.add_argument('--output-dir', default=".",
                       help="Directory for strain-specific FASTA files")
    ...
    strain_taxids = [tid for tid in all_descendants if rank_map.get(tid) in strain_ranks]
    
    # If no strains found, fall back to species level
    if not strain_taxids:
        species_ranks = {'species', 'subspecies'}
        species_taxids = [tid for tid in all_descendants if rank_map.get(tid) in species_ranks]
        strain_species_taxids = sorted(species_taxids)
        print(f"No strains found under taxid {args.taxid}, using {len(species_taxids)} species-level taxids")
    else:
