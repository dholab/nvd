...
def check_guard_list(labkey, guard_list_name, experiment_id, cluster_id) -> bool:
    ...
    try:
        logger.info(
            f"Checking guard list for experiment ID: {experiment_id}, cluster ID: {cluster_id}"
        )
        ...
        if result["rows"]:
            ...
            logger.info(
                f"Found existing entry: experiment_id={existing_row['experiment_id']}, cluster_id={existing_cluster_id}"
            )

            if str(existing_cluster_id) == str(cluster_id):
                logger.info(
                    f"Experiment ID {experiment_id} already registered for this cluster ({cluster_id}) - proceeding"
                )
                return True
            else:
                logger.error(
                    f"Experiment ID {experiment_id} is already in use by cluster {existing_cluster_id}"
                )
                return False
        else:
            # Experiment ID doesn't exist, add it to the guard list
            logger.info(
                f"No existing entry found. Registering experiment ID {experiment_id} for cluster {cluster_id}"
            )
            guard_row = {"experiment_id": experiment_id, "cluster_id": cluster_id}
        ...
        if (
            ...
            logger.info(
                "Note: Please ensure the guard list exists with columns: experiment_id (integer), cluster_id (text)"
            )
...
def main() -> None:
    ...
    if not check_guard_list(
        labkey, args.guard_list, args.experiment_id, args.cluster_id
    ):
        print()
        logger.info("=" * 80)
        ...
        logger.info(
            "   • To clear a stale entry, delete the row from the guard list in LabKey"
        )
        logger.info(
            "   • Guard list should have columns: experiment_id (integer), cluster_id (text)"
        )
        logger.info(
        ...
        print()
        logger.info(
            f"• Experiment ID {args.experiment_id} is registered for cluster {args.cluster_id}"
        )
