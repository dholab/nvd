...
def check_guard_list(labkey, guard_list_name, experiment_id, unique_id) -> bool:
    ...
    """
    try:
        logger.info(f"Checking guard list for experiment ID: {experiment_id} - {unique_id}")
        ...
        if result["rows"]:
            ...
            logger.info(f"Found existing entry: experiment_id={existing_row['experiment_id']}, unique_id={existing_unique_id}")
            
            if str(existing_unique_id) == str(unique_id):
                logger.info(f"Experiment ID {experiment_id} already registered for this cluster ({unique_id}) - proceeding")
                return True
            else:
                logger.error(f"Experiment ID {experiment_id} is already in use by cluster {existing_unique_id}")
                return False
        else:
            # Experiment ID doesn't exist, add it to the guard list
            logger.info(f"No existing entry found. Registering experiment ID {experiment_id} for cluster {unique_id}")
            guard_row = {
        ...
        if ("does not exist" in error_msg or 
            ...
            logger.info("Note: Please ensure the guard list exists with columns: experiment_id (integer), unique_id (text)")
...
def main() -> None:
    ...
    if not check_guard_list(labkey, args.guard_list, args.experiment_id, args.unique_id):
        print()
        logger.info("=" * 80)
        ...
        logger.info(f"   • To clear a stale entry, delete the row from the guard list ({args.guard_list}) in LabKey")
        logger.info("   • Guard list should have columns: experiment_id (integer), unique_id (text)")
        ...
        logger.info("=" * 80)
        print()
        logger.info(f"• Experiment ID {args.experiment_id} is registered for {args.unique_id}")
