...
def create_request(url: str, method: str, auth_header: str, 
...
def handle_upload_error(e: Exception, local_path: str, remote_url: str, 
                       auth_header: str) -> bool:
...
def upload_files_to_directory(directory_url: str, file_paths: List[str], 
                             auth_header: str, create_dir: bool = True) -> Dict[str, bool]:
...
Examples:
  ...
  python %(prog)s check some_folder/file.txt
    ...
    if args.command == 'mkdir':
        ...
        if len(args.args) < 2:
            print("Error: upload-dir requires at least 2 arguments: target_dir file1 [file2 ...]")
            sys.exit(1)
...
def debug_test():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--password':
        ...
        print(f"Password lengths - Hardcoded: {len(API_KEY)}, CLI: {len(password_from_cli)}")
        ...
        result1 = client1['upload'](test_file, urllib.parse.urljoin(BASE_URL, "debug1.txt"))
        print(f"Result: {'Success' if result1 else 'Failed'}")
        
        print("\nTesting upload with CLI client:")  
        result2 = client2['upload'](test_file, urllib.parse.urljoin(BASE_URL, "debug2.txt"))
