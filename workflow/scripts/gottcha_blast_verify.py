...
def classify_hits(hits_file, descendants, class_out):
    ...
    with open(class_out, "w", newline="") as outfh:
        ...
        for qid, hits in data.items():
            ...
            best = set(t for e, tids in hits if e == min_ev for t in tids)
            if best <= descendants:
...
def main():
    ...
    p.add_argument("--query",     required=True,
                   help="FASTA input for BLAST")
    p.add_argument("--db",        default="ref/core_nt",
                   help="BLAST DB prefix (e.g. ref/core_nt)")
    p.add_argument("--threads",   type=int, default=8,
                   help="BLAST threads")
    p.add_argument("--word_size", type=int, default=64,
                   help="BLAST word size")
    p.add_argument("--max_target_seqs", type=int, default=None,
                   help="Maximum target sequences to retrieve (omit for BLAST default)")
    p.add_argument("--target-taxid", required=True,
                   help="Parent taxID to include (plus all descendants)")
    p.add_argument("--hits-out",  default="hits.tsv",
                   help="Intermediate BLAST output")
    p.add_argument("--class-out", default="classification.tsv",
    ...
    run_blast(args.query, args.db, args.threads, args.word_size,
              args.max_target_seqs, args.hits_out)
