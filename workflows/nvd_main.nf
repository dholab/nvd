...
workflow NVD_MAIN {
  ...
  assert !(params.stream_sra && !params.experimental) : """
    ...
    Choose one:
      - Test paired ENA streaming: set experimental = true
  ...
  assert !(params.stream_sra && !params.skip_fastqc) : """
    ...
    stream_sra sends paired ENA reads directly into deacon through nuclease
    passthrough mode, so complete raw FASTQ files are not available for FastQC.
    ...
    Choose one:
      - Keep ENA streaming: set skip_fastqc = true
