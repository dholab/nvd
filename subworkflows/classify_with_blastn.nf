...
workflow CLASSIFY_WITH_BLASTN {
  ...
  main:
  // Keep one explicit channel emission per evidence-class batch that reaches
  ...
  ch_samples_after_megablast = ch_filtered_megablast
    ...
    .map { sample_id, evidence_class, megablast_hits, classified, pruned_contigs ->
      def needs_blastn = file(pruned_contigs).size() > 0
      def meta = [id: sample_id, evidence_class: evidence_class, blastn_needed: needs_blastn]
  ...
  ch_blastn_context = ch_samples_requiring_blastn.map { meta, megablast_hits, _classified, _pruned_contigs ->
    tuple(meta.id, meta.evidence_class, meta, megablast_hits)
  }

  BLASTN_CLASSIFY(
    ch_samples_requiring_blastn.map { meta, _megablast_hits, classified, pruned_contigs ->
      tuple(meta.id, meta.evidence_class, classified, pruned_contigs)
  ...
  ch_blastn_terminal = FILTER_NON_VIRUS_BLASTN_NODES.out
    ...
    .map { sample_id, evidence_class, blastn_hits, _meta, megablast_hits ->
      tuple(sample_id, evidence_class, [megablast_hits, blastn_hits])
    }
  ...
  ch_samples_skipping_blastn = ch_samples_after_megablast
    ...
    .map { meta, megablast_hits, _classified, _pruned_contigs ->
      tuple(meta.id, meta.evidence_class, [megablast_hits])
    }
  ...
  ch_sample_merged_inputs = ANNOTATE_LEAST_COMMON_ANCESTORS.out
    .map { sample_id, _evidence_class, batch_tsv -> tuple(sample_id, batch_tsv) }
    .groupTuple()
