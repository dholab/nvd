...
NVD_DEFAULT_REPO={nvd_repo}
export NVD_REPO="${{NVD_REPO:-${{NVD_PIPELINE_ROOT:-$NVD_DEFAULT_REPO}}}}"
...
def _generate_wrapper_script(nvd_repo: Path) -> str:
    ...
    return WRAPPER_TEMPLATE.format(
        version=__version__,
        date=datetime.now(UTC).strftime("%Y-%m-%d"),
        nvd_repo=shlex.quote(str(nvd_repo)),
    )
...
def _install_wrapper_script(
    ...
    with tempfile.NamedTemporaryFile(
        ...
        temporary_file.write(wrapper_content)
        temporary_path = Path(temporary_file.name)

    try:
        temporary_path.chmod(0o755)
        temporary_path.replace(wrapper_path)
    finally:
...
def _migrate_legacy_wrapper(nvd_repo: Path, wrapper_path: Path) -> bool:
    """Replace an unmodified setup-generated legacy wrapper."""
    ...
    legacy_pattern = re.escape(LEGACY_WRAPPER_TEMPLATE)
    legacy_pattern = legacy_pattern.replace(re.escape("{version}"), r"[^\n]+")
    legacy_pattern = legacy_pattern.replace(re.escape("{date}"), r"[^\n]+")
    legacy_pattern = legacy_pattern.replace(re.escape("{nvd_repo}"), r'[^"\n]+')
    if re.fullmatch(legacy_pattern, wrapper_content) is None:
        return False

    _install_wrapper_script(nvd_repo, wrapper_path.parent)
    return True
...
def _write_setup_conf(
    ...
    *,
    ...
    Existing setup metadata is preserved unless replacement is explicit.
    """
    conf_path = get_setup_conf_path()
    if (conf_path.exists() or conf_path.is_symlink()) and not overwrite:
        return conf_path
...
def setup(
    ...
    if (wrapper_path.exists() or wrapper_path.is_symlink()) and not force:
        if _migrate_legacy_wrapper(nvd_repo, wrapper_path):
            success(f"Legacy wrapper migrated at {wrapper_path}")
        else:
            warning(f"Keeping unrecognized wrapper script at {wrapper_path}")
    ...
    # Preserve existing setup metadata during wrapper-only migrations.
    setup_conf_path = get_setup_conf_path()
    setup_conf_existed = setup_conf_path.exists() or setup_conf_path.is_symlink()
    ...
    if setup_conf_existed and not force:
        info(f"Keeping existing setup config at {conf_path}")
    else:
