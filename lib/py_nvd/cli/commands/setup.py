...
WRAPPER_TEMPLATE = """\
#!/bin/bash
# NVD wrapper - runs nvd within its pixi environment
# Generated by: nvd setup v{version}
# Date: {date}

NVD_REPO="{nvd_repo}"
exec pixi run --manifest-path "$NVD_REPO/pyproject.toml" nvd "$@"
"""
...
def _prompt_state_dir(default: Path, non_interactive: bool) -> Path:
    ...
    console.print(f"State directory [{default}]: ", end="")
...
def shell_hook(
    ...
    # Determine completion environment variable based on shell
    if detected_shell == "bash":
        complete_var = "_NVD_COMPLETE=bash_source"
    else:  # zsh
        complete_var = "_NVD_COMPLETE=zsh_source"
